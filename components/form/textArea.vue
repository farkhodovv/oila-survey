<template>
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-md-8">
        <div class="test">
          <div class="test__header">
            <div class="counter">
              <span>3</span><span class="gray-text">/24</span>
            </div>
            <h5>So’rovnomalar qanchalik foydali
              bo‘lyapti ?</h5>
          </div>
          <div class="test-body mt-5">
            <b-form-input v-model="text" placeholder="Yozma shaklida javob bering (misol: 25)"></b-form-input>
          </div>
          <div class="test__footer d-flex justify-content-end mt-5">
            <button class="align-items-center btn-blue">Keyingisi
              <svg class="ml-2" width="20" height="20" viewBox="0 0 20 20" fill="none"
                   xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M7.99989 18H6.66656C6.40922 18 6.17456 17.8513 6.06389 17.6187C5.95322 17.386 5.98722 17.1107 6.15056 16.9107L11.8052 10L6.15056 3.08867C5.98722 2.88933 5.95389 2.614 6.06389 2.38067C6.17389 2.14733 6.40922 2 6.66656 2H7.99989C8.19989 2 8.38922 2.09 8.51589 2.24467L14.5159 9.578C14.7166 9.824 14.7166 10.1767 14.5159 10.4227L8.51589 17.756C8.38922 17.91 8.19989 18 7.99989 18Z"
                  fill="white"/>
              </svg>
            </button>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'test',
  data() {
    return {
      text: '',
    }
  }
}
</script>
<style scoped lang="scss">
.multiple {
  margin: 20px 0;
}
</style>
